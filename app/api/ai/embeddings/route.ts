...
function normalizeSource(v: unknown): IngestSource | null {
  if (v === "sections" || v === "projects" || v === "blogs" || v === "resume" || v === "all") {
    return v;
  }
...
async function persistEmbeddings(chunks: ChunkDoc[], vectors: number[][]) {
   ...
   * Later store in Supabase/pgvector (chunk text + vector + metadata).
   */
  void chunks;
  void vectors;
...
export async function POST(req: Request) {
  ...
  if (!source) {
    return jsonError(400, "Invalid or missing field: source (sections|projects|blogs|resume|all)");
  }
  ...
  if (processed === 0) {
    warnings.push("No documents found for ingestion (current data source returns empty).");
    warnings.push("Wire fetchDocsForIngestion() to your CMS/Supabase content to enable real embeddings.");
  }

  if (chunks.length === 0) {
    warnings.push("No chunks produced. Check source data or chunking settings.");
  }
  ...
  if (fullReindex) {
    warnings.push("fullReindex=true requested. Deletion/rebuild will be implemented once DB storage is wired.");
  }
